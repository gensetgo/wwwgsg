...
<% 	
	...
	int error=0;
	String hashString="";
	String productinfo=request.getAttribute("productinfo")+" - ";
 	String orderId=request.getAttribute("orderId")+"";
 String succURL = "http://www.gensetgo.com/orderConfirmation.jsp?id="+request.getAttribute("orderId")+"";
String service_provider="payu_paisa";
	...
	if(empty(params.get("hash")) && params.size()>0)
	{
		...
		if( empty(merchant_key)
			|| empty(txnid)
			|| empty(params.get("item_grand_tot"))
			|| empty(params.get("cust_name"))
...
<input type="hidden" name="key" value="<%= merchant_key %>" />
      ...
      <table>
        ...
        <tr>
          <td>Amount: </td>
          <td><input name="amount" value="<%= (empty(params.get("item_grand_tot"))) ? "" : params.get("item_grand_tot") %>" /></td>
          <td>First Name: </td>
          <td><input name="firstname" id="firstname" value="<%= (empty(params.get("cust_name"))) ? "" : params.get("cust_name") %>" /></td>
        </tr>
        <tr>
          <td>Email: </td>
        ...
        <tr>
          <td>Product Info: </td>
          <td colspan="3"><input name="productinfo" value="<%=productinfo%>" size="64" /></td>
        </tr>
        <tr>
          <td>Success URI: </td>
          <td colspan="3"><input name="surl" value="<%= succURL %>" size="64" /></td>
        </tr>
        <tr>
          <td>Failure URI: </td>
          <td colspan="3"><input name="furl" value="<%= succURL %>" size="64" /></td>
        </tr>
        <tr>
